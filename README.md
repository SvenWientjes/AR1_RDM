...
document](https://github.com/SvenWientjes/AR1_DDM) instead. It is
recommended to scroll through that tutorial first, because some of the
concepts introduced there are assumed prior knowledge for following this
tutorial.
...
evidence in favor of a particular option, mostly resembling $vd$. Also,
in the DDM, by nature of there being only a single drifting state, there
can be no difference in variability analogous to $sd$ in the RDM.
...
these simulations. It is a bit inefficiently programmed, but works
robustly. Again, you could instead download my simulations here and
unpack them as `simulations/RDM_simulations.csv`.
...
Compared to the fits with the (AR1) DDM ([see
here](github.com/SvenWientjes/AR1_DDM)) we obtain much better fits to
the marginal RT distributions.
...
Here, the solid lines correspond to the empirical CAF and the dashed
lines correspond to the posterior predicted CAF from the RDM. We can see
that the RDM tends to have a decreasing CAF. This is because as the
decision process takes longer, the probability of the incorrect (on
average slower) racer winning increases. This happens in the RDM because
the racers are not in competition. By contrast, in the DDM, there is
*strong competition*, because each piece of evidence in favor of one
option counts as a piece of evidence against the other option. This
tug-of-war leads to a flatter CAF for the DDM.

Now we see that the RDM provides better overall marginal fits to the RT
distributions, but does not provide quite the right CAF. The next
question to ask is whether between-trial fluctuations can help improve
the fit to the CAF.

## Fitting and exploring the AR1-RDM model

To fit the parameters of the RDM as AR(1) processes, we can rely on the
same trick as we did for the DDM. Interestingly, now we have an
additional parameter that we can let fluctuate. We will fluctuate
parameters $[B,db,V,db]$. In principle, diffusion-noise related
parameters could also be made to fluctuate.
...
solves the issue, perhaps with a higher `adapt_delta`. For now, lets
continue to see how the model performs.
...
We can visualize the marginal RT distributions again, and see we
somewhat improved for one of these example participants! (purple)
...
This is good! We capture the same qualitative pattern of both fast and
slow errors, although the pattern is somewhat attenuated in our
posterior predictions.

### Modeling correlated error patterns

One key pattern of AR(1)-like fluctuataing dynamics, identified by
Vloeberghs et al. (2026), is that error identities correlate over time.
Fast errors are more likely followed by fast errors rather than slow
errors, and errors of response 1 are more likely followed by errors of
response 1 rather than of response 2. Here we visualize both the
empirical and posterior-predicted relationship between successive
errors:
...
![](README_files/figure-commonmark/unnamed-chunk-15-2.png)

We can see that the AR1-RDM is highly accurate in recapitulating the
correlated error structure, much more so than the AR1-DDM was!

## Conclusion

The AR1-RDM can perform much better than the AR1-DDM it seems, although
it does show some instability for some participants. Future work should
check if divergent fits can be fixed by refitting, or if reduced models
need to be fit for the participants where stable convergence failed.
